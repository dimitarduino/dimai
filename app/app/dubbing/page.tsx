...
import { deductUserCredits, insertDubbingVideo } from "@/app/app/_actions/dashboard-data";
...
export default function DubbedVideo() {
    ...
    const makeDubVideo = async (videoUrl = ``) => {
        ...
        const data = await axios.post("/api/extract-audio", {
            ...
            const slednoPoeni = await deductUserCredits(20);

            setUserDetail((prev) =>
                prev ? { ...prev, credits: slednoPoeni } : prev
            );
    ...
    const handleUpload = async () => {
        ...
        if (!proveriPoeni(userDetail?.credits ?? 0, 20)) {
        ...
        <div className="w-full flex">
            <div className="flex dark:bg-neutral-900 bg-white py-12 rounded-xl shadow-sm px-10 mt-4 flex-col max-w-4xl mx-auto space-y-4 p-4">
                ...
                <Dialog open={!!openedResult} onOpenChange={setOpenedResult}>
                    <DialogContent className="flex w-full [&>button]:hidden max-w-md sm:max-w-md max-h-[80dvh] sm:max-h-[90dvh] overflow-y-auto flex-col z-150">
                        <DialogHeader>
