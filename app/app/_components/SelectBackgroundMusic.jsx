...
function SelectBackgroundMusic({ value, onUserSelect, name = "backgroundMusicId" }) {
  const audioRef = useRef(null);
  const [previewing, setPreviewing] = useState(false);
  const selected = SHORTS_BACKGROUND_TRACKS.find((t) => t.id === value) || SHORTS_BACKGROUND_TRACKS[0];
  ...
  return (
    <div className="w-full space-y-2">
      ...
      <p className="text-gray-500 dark:text-neutral-300">
        Royalty-free tracks for TikTok-style shorts. Mixes under your voice in the final export. Check license notes before posting publicly (especially CC BY tracks).
      </p>
      <Select
        value={value}
        onValueChange={(id) => onUserSelect(name, id)}
      >
