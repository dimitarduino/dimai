...
import { Crown, DollarSign, PlusCircleIcon } from "lucide-react";
  ...
  return (
    <div className="p-3 sticky top-0 left-0 bg-white md:bg-transparent dark:bg-zinc-950 z-100 w-full px-5 flex items-center items-center justify-between">
      ...
      <div className="flex gap-3">
        <button
          onClick={toggleTheme}
          className="bg-transparent dark:hover:bg-neutral-800 hover:bg-neutral-100 cursor-pointer px-4 rounded-full"
        >
          {isDark ? <Moon size={20} className="" /> : <Sun size={20} className="" />}
        </button>
        <Link
          href="/app/upgrade"
          className="flex items-center gap-2 bg-neutral-200 dark:bg-neutral-700 rounded-md px-3 py-2 cursor-pointer hover:bg-neutral-300"
        >
          <div className="bg-primary p-1 rounded-full">
            <DollarSign className="font-bold text-white" aria-hidden size={12} />
          </div>
          <span className="font-bold text-primary">{userDetail?.credits}</span>
