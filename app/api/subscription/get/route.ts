...
const fsAny = freemius as Record<string, any>;
...
export async function POST(req) {
  try {
    ...
    let freemiusSubscription: Record<string, any> | null = null;
    let currentPlan: Record<string, any> | null = null;
    
    try {
      // Use the correct Freemius SDK method to get subscriptions by email
      const subscriptions = await fsAny.purchase.retrieveSubscriptionsByEmail(email);
      ...
      if (subscriptions && subscriptions.length > 0) {
        ...
        if (freemiusSubscription) {
          ...
          if (freemiusSubscription.plan_id) {
            try {
              const plan = await fsAny.api.plans.retrieve({
